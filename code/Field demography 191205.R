...
for (i in 1:nrows){
  ...
  for (j in 1:nb){
    ...
    massContrib.boot.mean[j]<-100*germ.boot.mean[j]*surv.boot.mean[j]*mass.boot.mean[j] #Directly estimated contributed mass shows strong kurtosis
  }
  ...
  estimates$massContrib.mean[i]<-mean(massContrib.boot.mean)
  ...
  estimates$germ.boot.95CIhi[i]<-quantile(germ.boot.mean,probs=0.975)
  estimates$surv.boot.95CIlo[i]<-quantile(surv.boot.mean,probs=0.025)
  estimates$surv.boot.95CIhi[i]<-quantile(surv.boot.mean,probs=0.975)
  estimates$mass.boot.95CIlo[i]<-quantile(mass.boot.mean,probs=0.025)
  estimates$mass.boot.95CIhi[i]<-quantile(mass.boot.mean,probs=0.975)
  estimates$massContrib.95CIlo[i]<-quantile(massContrib.boot.mean,probs=0.025)
  estimates$massContrib.95CIhi[i]<-quantile(massContrib.boot.mean,probs=0.975)
}#Takes a few seconds
...
for (i in 1:nrows){
  ...
  massContrib.boot.mean<-NULL
  for (j in 1:nb){
    germ.boot.mean[j]<-mean(na.omit(sample(subdata$Germ,replace=T)))
    surv.boot.mean[j]<-mean(sample(subdata$Surv,replace=T))
    mass.boot.mean[j]<-mean(sample(na.omit(subdata$Abv),replace=T))#Dead plants not included in mass calculation
    massContrib.boot.mean[j]<-100*germ.boot.mean[j]*surv.boot.mean[j]*mass.boot.mean[j] #Directly estimated contributed mass shows strong kurtosis
  }
...
ggplot(data=estimates, aes(x=Type, y=massContrib.mean, color=Treatment, shape=Donorspp))+
  facet_grid(~Transplant)+
  geom_pointrange(aes(ymin=massContrib.mean-massContrib.95CIlo, ymax=massContrib.95CIhi+massContrib.mean)
                  , position = position_dodge(width = 1))+  #Errors asymmetric
  scale_color_manual(values=c("#FF0000", "#00A08A", "#F2AD00"))+
  xlab("Microsite")+ylab("Estimated shoot biomass contribution from 100 seeds")+
  theme_bw()
